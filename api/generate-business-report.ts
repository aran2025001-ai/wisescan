...
【七、JSON输出格式】
{
  ...
  "share_card": {
    "project_name": "■ 前端已提供项目名称，直接原样填入本字段，禁止修改、禁止重新提取、禁止替换为'未命名项目'或其他值。",
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const userPrompt = `请分析以下加密项目的商业模式，并严格按照 System Prompt 的 JSON 格式输出结果。${projectNameTrimmed ? '\n⚠️ 重要：share_card.project_name 必须原样填入"' + projectNameTrimmed + '"，禁止修改、禁止重新提取。' : ''}
