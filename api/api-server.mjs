...
async function handleGenerateReport(req, res) {
  ...
  const body = await readBody(req);
  const { project_name, contract_address, project_id, user_notes, user_notes_images, website, community, whitepaper, quick_verify, user_address, frontend_verified } = body;
  ...
  // 📎 项目补充信息：官网、社交、白皮书（用户在前端表单填写）
  const projectLinks = [
    website ? `官网：${website}` : '',
    community ? `社交链接：${community}` : '',
    whitepaper ? `白皮书：${whitepaper}` : '',
  ].filter(Boolean).join('\n');
  const finalNotes = effectiveNotes + (projectLinks ? '\n\n## 用户提交的项目链接\n' + projectLinks : '');
...
【输出JSON格式】
必须严格按以下结构输出。没有就留空数组[]或null。
⚠️ special_switches 每当项目有可切换的功能（如涡轮加速、加倍功能等）必须填入，不能省略。
⚠️ ⚠️ visualization_tree 是**必填字段**！只要项目提到"返佣/直推/间推/对碰奖/代/层/点位/推荐"中的任意一个关键词，就**必须**生成树形结构（根节点为"你" → 下一级为直接推荐 → 再下级为间接推荐）。这是**强制要求，违反即回答失败**。只有纯静态收益/矿机/持币生息等完全没有层级模式时才留 null。
{
  ...
  "visualization_hint": "根据返佣层级和点位关系构建树。根节点是你，下面是直接推荐，每个推荐下再分下级。要包含 description 字段说明节点角色。",
  "visualization_tree": [
    { "name": "你（共识账户）", "description": "你的位置", "children": [
      { "name": "B1（共识）", "description": "直接推荐1", "children": [
        { "name": "C1", "description": "共识账户" },
        { "name": "C2", "description": "共识账户" }
      ]},
      { "name": "B2（共识）", "description": "直接推荐2", "children": [
        { "name": "C3", "description": "共识账户" }
      ]},
      { "name": "B3（共识）", "description": "直接推荐3" },
      { "name": "B4（共识）", "description": "直接推荐4" },
      { "name": "B5（共识）", "description": "直接推荐5", "children": [
        { "name": "C4", "description": "共识账户" },
        { "name": "C5", "description": "共识账户" }
      ]}
    ]}
  ],
...
async function handleGenerateBusinessReport(req, res) {
  ...
  const body = await readBody(req);
  const { project_name, rule_text, rules_text, investment, referrals, user_notes, user_notes_images, website, community, whitepaper, user_address, contract_address } = body;
...
async function handleChat(req, res) {
  ...
  try {
    const body = await readBody(req);
    const { project_name, contract_address, message, chat_history, user_address, conversation_count, is_paid, page, report_data: inlineReportData, token_price, token_symbol, user_notes, website, community, whitepaper } = body;
...
${user_notes ? `\n【用户补充说明】${user_notes}` : ''}${website ? `\n【官网】${website}` : ''}${community ? `\n【社交媒体】${community}` : ''}${whitepaper ? `\n【白皮书】${whitepaper}` : ''}
...
【当前项目/规则】${project_name || '用户提交的商业模式'}
【证据】${evidenceContext}
${user_notes ? `【用户补充说明】${user_notes}` : ''}${website ? `【官网】${website}` : ''}${community ? `【社交媒体】${community}` : ''}${whitepaper ? `【白皮书】${whitepaper}` : ''}
