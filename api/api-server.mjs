...
⚠️ visualization_tree 当规则文本中有返佣层级/点位关系时，必须生成树形结构数据；没有层级关系才留 null。
{
  ...
  // ⚠️ visualization_tree：有返佣层级时必须根据规则文本中的实际层级和推荐人数生成树形结构，
  //    不要随意缩减层级或节点数。规则文本中没有提及层级关系时才留 null。
  //    示例展示的是有多层返佣和多个节点的结构，实际项目请根据规则内容生成。
  "visualization_tree": [
