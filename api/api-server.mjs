...
import { analyzeImage } from './utils/multimodal.mjs';
...
【七、JSON输出格式】
{
  ...
  "share_card": {
    "project_name": "项目名称提取规则（按优先级执行）：1) 如果用户在前端填写了项目名称，直接使用该值；2) 否则，从规则文本中提取项目名称，项目名称通常出现在规则文本的开头，格式如'XX项目'、'XX平台'、'XX模式'，或包含大写字母缩写（如'MY'、'MYBX'、'TRADOOR'）；3) 如果规则文本开头是纯规则描述（如'投资100U起...'），则从文本中检索大写字母缩写或项目名关键词；4) 如果以上都提取不到，返回'未命名项目'。禁止返回'用户自定义'或其他占位符。",
...
async function handleGenerateBusinessReport(req, res) {
  ...
  const body = await readBody(req);
  const { project_name, rule_text, rules_text, investment, referrals, user_notes, user_address, contract_address } = body;
...
${projectNameTrimmed ? '⚠️ 重要：share_card.project_name 必须原样填入"' + projectNameTrimmed + '"，禁止修改、禁止重新提取。' : ''}

项目名称：${projectNameTrimmed || '（未提供，请从规则文本中提取）'}
  ...
  try {
    ...
    try {
      ...
      if (dbErr) {
        ...
        console.log('💾 business_reports 存储 OK');
      }
    } catch (dbErr) {
