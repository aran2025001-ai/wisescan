...
async function handleVerifyPayment(req, res) {
  ...
  try {
    const body = await readBody(req);
    const { txHash, reportType, userAddress, projectId, couponId } = body;
    ...
    if (!tx) return jsonRes(res, 404, { error: 'Transaction not found on chain' });
    ...
    let receipt;
    try {
      receipt = await provider.waitForTransaction(txHash, 3, 30000);
    } catch {
    ...
    // 7. 🔐 计算金额（从环境变量读，不信任前端）
    let baseAmount = getPriceAmount(reportType);
