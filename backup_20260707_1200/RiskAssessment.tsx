...
export default function RiskAssessment() {
  ...
  // 每次导航到本页面时的滚动行为：有已持久化的消息则滚到底部，否则重置到顶部
  useEffect(() => {
    const hasMessages = messages.length > 0 && messages.some(m => m.type === 'user')
    if (hasMessages) {
      // 有聊天记录 → 等 DOM 渲染完成后滚到底部
  ...
  // ⚠️ 核心自动滚动 effect：当有新消息时滚到底部
  // 注意：此 effect 只处理"消息更新时滚到底部"，不处理"初始定位"。
  //     "初始定位"（老用户无聊天记录时定位到表单位置）由下方的独立 effect 处理。
  useEffect(() => {
  ...
  // 🎯 初始定位 effect：仅在页面加载时执行一次
  // 目的：老用户无聊天记录时，定位到表单位置（ready-prompt 气泡），
  //       而不是页面顶部或底部。
  // 独立于上面的 useEffect([messages])，避免干扰按钮点击后的滚动行为。
  useEffect(() => {
    const t = setTimeout(() => {
      const hasChatHistory = messages.some(m => m.type === 'user')
      if (!hasChatHistory && messages.length > 0) {
        // 老用户无聊天记录 → 到 ready-prompt 表单位置（用户截图中的位置）
