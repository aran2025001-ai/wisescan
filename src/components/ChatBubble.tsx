...
export function ChatBubble({ role, content, time }: ChatBubbleProps) {
  ...
  return (
    <div className={`flex gap-3 px-4 py-2 ${isAi ? 'justify-start' : 'justify-end'}`}>
      {/* AI Avatar */}
      {isAi && (
        <div className="flex-shrink-0 flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-cyan-400 text-zinc-50 text-xs font-bold">
          洞察
        </div>
      )}
      ...
      {/* User Avatar */}
      {!isAi && (
        <div className="flex-shrink-0 flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-purple-500 to-pink-400 text-zinc-50 text-xs font-bold">
          我
        </div>
      )}
    </div>
